...
provisioner "file" {
    ...
    provisioner "remote-exec" {
        inline = [
         ...
         "sudo docker run --net host --privileged -e USERNAME=${var.USERNAME} -e PASSWORD=${var.PASSWORD} -e ETCD_DISCOVER=discover.blue-etcd.shared.prsn-dev.io -e NAMESPACE=tsst2 -e CORE=yes --name vrouter -itd gcr.io/pearson-techops/cosmos/vrouter:COS-29",
