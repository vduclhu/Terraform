...
provisioner "file" {
    ...
    provisioner "remote-exec" {
        inline = [
           ...
           "echo sudo /tmp/autoupdate.sh ${var.USERNAME} ${var.PASSWORD} ${var.ETCD_HOST} ${aws_route_table_association.us-west-2a-public.route_table_id} ${var.NAME_SPACE} ${aws_subnet.us-west-2a-public3.id} | at now + 2 minute",
...
provisioner "file" {
    ...
    provisioner "remote-exec" {
        inline = [
           ...
           "echo Y | sudo apt-get install jq",
           "chmod +x /tmp/autoupdate.sh",
           "echo sudo /tmp/autoupdate.sh ${var.USERNAME} ${var.PASSWORD} ${var.ETCD_HOST} ${aws_route_table_association.us-east-2a-public.route_table_id} ${var.NAME_SPACE} ${aws_subnet.us-east-2a-public4.id} | at now + 2 minute",
